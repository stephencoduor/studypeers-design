...
// API ValidateOtp Controller
class ValidateOtp extends CI_Controller {
  public function __construct()
    ...
    public function index()
    {
        ...
        $data['otp']            = trim($requestJson[APP_NAME]['otp']);  
              ...
              if(!empty($headers['Deviceid']) && !empty($headers['Package'] && !empty($headers['Devicetype']))){
                if(($headers['Package'] == strtolower(md5(PACKAGE)) || $headers['Package'] == strtoupper(md5(PACKAGE)))){
                            if ($resultJson == 1) { 
                                ...
                                $chk = $this->ApiModel->validateOtp($userDetail['id'], $data['otp']);
                                
                                    if($chk != 0){
                                        $response['user_id'] = $userDetail['id'];
                                        generateServerResponse('1','106', $response);

                                    }else{
                                        generateServerResponse('0','107');
                                    }
                            }else{

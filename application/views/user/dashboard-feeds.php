...
?>
		...
		<?php } } else if($value['reference'] == 'studyset') {  
			...
			if($user_id == $studyset_detail['user_id']) {
				$chk_view = 1;
			} else {
					...
					$university = $this->db->get_where('university', array('university_id' => $user_info['intitutionID']))->row_array(); ?>
                        ...
                        <div class="documentName">
                            ...
                            <?php if($extn == 'pdf') { ?>
								...
								<img src="<?php echo base_url(); ?>assets_d/images/pdf.svg" alt="pdf"> 
								<a class="preview-doc" data-toggle="modal" data-id="<?php echo $document_detail['id']; ?>" data-target="#documentPreview"><?php echo $document_detail['featured_image']; ?></a>
                            <?php } else { ?>
                                <img src="<?php echo base_url(); ?>assets_d/images/pdf.svg" alt="pdf"> <a class="preview-doc" data-toggle="modal" data-id="<?php echo $document_detail['id']; ?>" data-target="#documentPreview"><?php echo $document_detail['featured_image']; ?>
                            <?php } ?>
                        </div>
						<div class="socialStatus">
							<div class="leftStatus">
...
<?php if($loadMore == 1) { ?>
	<div class="loadMoreWrapper loadmore" id="loadmore_<?= $nextOffset; ?>">
		<button type="button" onclick="loadDashboardFeeds(<?= $nextOffset; ?>);"> Load More</button>
	</div>
<?php } else { ?>

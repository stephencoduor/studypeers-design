...
if (! function_exists('is_job_wishlisted')) {
  function is_job_wishlisted($listing_id = '') {
    $CI =&  get_instance();
    $CI->load->database();
    if ($CI->session->userdata('is_logged_in') != 1 ) {
      return false;
...
if (! function_exists('is_rental_wishlisted')) {
  function is_rental_wishlisted($listing_id = '') {
    $CI =&  get_instance();
    $CI->load->database();
    if ($CI->session->userdata('is_logged_in') != 1 ) {
      return false;
    ...
    $user_details = $CI->db->get_where('user', array('id' => $CI->session->userdata('user_id')))->row_array();
    if ($user_details['rental_wishlist'] != "") {

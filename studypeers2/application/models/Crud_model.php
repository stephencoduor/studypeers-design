...
class Crud_model extends CI_Model {
  ...
  function add_category() {
    ...
    if ($_FILES['icon_class']['name'] == "") {
        $data['icon_class'] = 'thumbnail.png';
      }else {
        $data['icon_class'] = md5(rand(10000000, 20000000)).'.jpg';
        move_uploaded_file($_FILES['icon_class']['tmp_name'], 'uploads/business_icon_class/'.$data['icon_class']);
      }
    if ($data['parent'] == 0) {
  ...
  function edit_category($category_id = "") {
    ...
    if ($_FILES['icon_class']['name'] == "") {
        ...
        $data['icon_class'] = md5(rand(10000000, 20000000)).'.jpg';
        move_uploaded_file($_FILES['icon_class']['tmp_name'], 'uploads/category/'.$data['icon_class']);
      }
    $data['name'] = sanitizer($this->input->post('name'));
  ...
  function add_amenity() {
    ...
    $data['slug'] = slugify(sanitizer($this->input->post('name')));
    if ($_FILES['amenity_thumbnail']['name'] == "") {
        $data['amenity_image'] = 'thumbnail.png';
      }else {
        $data['amenity_image'] = md5(rand(10000000, 20000000)).'.jpg';
        move_uploaded_file($_FILES['amenity_thumbnail']['tmp_name'], 'uploads/amenities/'.$data['amenity_image']);
      }
...
function add_listing() {
  ...
  $data['locality_id'] = sanitizer($this->input->post('locality_id'));
  ...
  $data['additional_phone'] = sanitizer($this->input->post('phone2'));
...
function delete_from_table($table_name = "", $id, $par="id") {
...
public function update_system_settings() {
  ...
  $data['description'] = sanitizer($this->input->post('address'));
...
public function update_frontend_settings() {
  ...
  $data['description'] = sanitizer($this->input->post('about_us'));
  $this->db->where('type', 'about_us');
  $this->db->update('frontend_settings', $data);

  $data['description'] = sanitizer($this->input->post('terms_and_condition'));
  $this->db->where('type', 'terms_and_condition');
  $this->db->update('frontend_settings', $data);

  $data['description'] = sanitizer($this->input->post('privacy_policy'));
  $this->db->where('type', 'privacy_policy');
  $this->db->update('frontend_settings', $data);

  $data['description'] = sanitizer($this->input->post('faq'));
...
function get_user_wise_job_wishlist() {
  $wishlists = array();
  $user_details = $this->db->get_where('user', array('id' => $this->session->userdata('user_id')))->row_array();
  $wishlists = json_decode($user_details['job_wishlist']);
  if (count($wishlists) > 0) {
    $this->db->where_in('id', $wishlists);
    return $this->db->get('job_listing')->result_array();
  }else {
    return array();
  }
}

function get_user_wise_rental_wishlist() {
  $wishlists = array();
  $user_details = $this->db->get_where('user', array('id' => $this->session->userdata('user_id')))->row_array();
  $wishlists = json_decode($user_details['rental_wishlist']);
  if (count($wishlists) > 0) {
    $this->db->where_in('id', $wishlists);
    return $this->db->get('rental_listing')->result_array();
  }else {
    return array();
  }
}
...
function create_package_purchase_history($payment_method = "", $user_id = "", $package_id = "", $paid_amount = "") {
  ...
  $data['expired_date']   = strtotime("+30 day", $purchase_date);
...
function get_stripe_supported_currencies() {
  ...
  function curl_request($code = '') {
    ...
    //job
    function get_job_categories($category_id = 0) {
    if ($category_id > 0) {
      $this->db->where('id', $category_id);
    }
    return $this->db->get('job_category');
  }
  ...
  function edit_job_category($category_id = "") {
    $data['name'] = sanitizer($this->input->post('name'));
    $data['slug'] = slugify(sanitizer($this->input->post('name')));
    if ($_FILES['category_thumbnail']['name'] != "") {
      $data['thumbnail'] = md5(rand(10000000, 20000000)).'.jpg';;
  ...
  function get_job_listings($listing_id = 0) {
    if (strtolower($this->session->userdata('role')) != 'admin') {
      $this->db->where('user_id', $this->session->userdata('user_id'));
    }
    if ($listing_id > 0) {
    ...
    return $this->db->get('job_listing');
  }

  function add_job_listing() {
  
    $data['name'] = sanitizer($this->input->post('title'));
    $data['company'] = sanitizer($this->input->post('company'));
    $data['description'] = sanitizer($this->input->post('description'));
    $data['country_id'] = sanitizer($this->input->post('country_id'));
    $data['city_id'] = sanitizer($this->input->post('city_id'));
    $data['locality_id'] = sanitizer($this->input->post('locality_id'));
    ...
    $data['no_of_vacancy'] = sanitizer($this->input->post('no_of_vacancy'));
    $data['min_education'] = sanitizer($this->input->post('min_education'));
    $data['salary_to'] = sanitizer($this->input->post('salary_to'));
    $data['salary_from'] = sanitizer($this->input->post('salary_from'));
    $data['experience_to'] = sanitizer($this->input->post('experience_to'));
    $data['experience_from'] = sanitizer($this->input->post('experience_from'));
  ...
  function update_job_listing($listing_id = "") {
    $listing_details      = $this->crud_model->get_job_listings($listing_id)->row_array();
  ...
  function update_job_listings_single_column($column_name = "", $value = "", $listing_id) {
    $data = array(
      $column_name => $value
    );
    $this->db->where('id', $listing_id);
  ...
  function filter_job_listing_table($data = array()) {
  if ($data['status'] != 'all')
  $this->db->where('status', $data['status']);
 ...
 public function get_job_listing_details($listing_id = "", $attribute = "") {
    $this->db->where('id', $listing_id);
    if ($attribute != "") {
      $this->db->select($attribute);
    }
...
function add_rental_listing(){ 
  ...
  $data['email'] = sanitizer($this->input->post('email'));
  $data['phone'] = sanitizer($this->input->post('phone'));
  $data['rental_type'] = sanitizer($this->input->post('rental_type'));
  ...
  $this->db->insert('rental_listing', $data);
  $listing_id = $this->db->insert_id();
  $time_config['listing_id'] = $listing_id;
  $this->db->insert('time_configuration', $time_config);

  // Add listing inner details data
  $this->add_listing_type_wise_details(sanitizer($this->input->post('rental_type')), $listing_id);
  $this->session->set_flashdata('flash_message', get_phrase('listing_added_successfully'));
}

function get_rental_listings($rental_id = 0) {
  if (strtolower($this->session->userdata('role')) != 'admin') {
    $this->db->where('user_id', $this->session->userdata('user_id'));
  }
  if ($rental_id > 0) {
  ...
  return $this->db->get('rental_listing');
}

function update_rental_listing($listing_id = "") {
  $listing_details      = $this->crud_model->get_rental_listings($listing_id)->row_array();
  ...
  $this->db->where('id', $listing_id);
  $this->db->update('rental_listing', $data);
 
  // remove the existing listing details from other tables
  $this->session->set_flashdata('flash_message', get_phrase('listing_updated_successfully'));
}

function get_rental_categories($category_id = 0) {
    if ($category_id > 0) {
      $this->db->where('id', $category_id);
    }
    return $this->db->get('rental_category');
}

function add_rental_category() {
    $data['name'] = sanitizer($this->input->post('name'));
    $data['slug'] = slugify($this->input->post('name'));
    ...
    $this->db->insert('rental_category', $data);

  }

  function edit_rental_category($category_id = "") {
    $data['name'] = sanitizer($this->input->post('name'));                                                                                                                                                                                                       
    $data['slug'] = slugify(sanitizer($this->input->post('name')));
    if ($_FILES['category_thumbnail']['name'] != "") {
      $data['thumbnail'] = md5(rand(10000000, 20000000)).'.jpg';;
    ...
    $this->db->where('id',$category_id);
    $this->db->update('rental_category', $data);
  }

  public function get_rental_listing_details($listing_id = "", $attribute = "") {
    $this->db->where('id', $listing_id);
    if ($attribute != "") {
      $this->db->select($attribute);
    }
  ...
  function filter_rental_listing_table($data = array()) {
    if ($data['status'] != 'all')
    $this->db->where('status', $data['status']);
    if ($data['user'] != 'all')
    $this->db->where('user_id', $data['user']);
  ...
  function filter_rental_listing_table_for_user($data = array()) {
    if ($data['status'] != 'all'){
    $this->db->where('status', $data['status']);}
  ...
  function update_rental_listings_single_column($column_name = "", $value = "", $listing_id) {
    $data = array(
      $column_name => $value
    );
    $this->db->where('id', $listing_id);
  ...
  function locality_details($locality_id = 0) {
    if ($locality_id > 0) {
      $this->db->where('id', $locality_id);
    }
    return $this->db->get('locality');
  }
  function update_localities($locality_id = 0) {
    $data['name'] = sanitizer($this->input->post('name'));
    $data['slug'] = slugify(sanitizer($this->input->post('name')));
    $data['country_id'] = sanitizer($this->input->post('country_id'));
    $data['city_id'] = sanitizer($this->input->post('city_id'));
...
function get_locality_list_by_city_id($city_id = 0) {
    $this->db->where('city_id', $city_id);
    return $this->db->get('locality');
  }

	function add_banners() {
    $data['type'] = sanitizer($this->input->post('name'));
    $data['listingID'] = sanitizer($this->input->post('listing'));
      if ($_FILES['banner_thumbnail']['name'] == "") {
        $data['banner_image'] = 'thumbnail.png';
      }else {
        $data['banner_image'] = md5(rand(10000000, 20000000)).'.jpg';
        move_uploaded_file($_FILES['banner_thumbnail']['tmp_name'], 'uploads/banners/'.$data['banner_image']);
      }
    $this->db->insert('banner', $data);
    
  }
  function update_banners($banner_id = "") {
      $data['type'] = sanitizer($this->input->post('name'));
      $data['listingID'] = sanitizer($this->input->post('listing'));
      if ($_FILES['banner_thumbnail']['name'] != "") {
        $data['banner_image'] = md5(rand(10000000, 20000000)).'.jpg';;
  ...
  function banner_details($banner_id = 0) {
    if ($banner_id > 0) {
      $this->db->where('bannerID', $banner_id);
    }
    return $this->db->get('banner');
  }
  function transaction_list($user_id = 0) {
    if ($user_id > 0) {
      $this->db->where('userID', $user_id);
    }
    return $this->db->get('transactions');
  }
  function redeem_list($user_id = 0) {
    if ($user_id > 0) {
      $this->db->where('id', $user_id);
    }
    return $this->db->get('user_redeem');
  }
  function update_redeem($redeem_id = "",$user_id = "") {
     $data = array(
        'notes' =>sanitizer($this->input->post('note')),
        'status' => sanitizer('completed')
      );
    
    $this->db->where('id',$redeem_id); 
    $this->db->update('user_redeem', $data);
    
  }
  function reject_redeem($redeem_id = "",$user_id = "") {
     $d = $this->db->update('user_redeem', array('id'=>$redeem_id));
     
     $this->db->where('id',$user_id);
     $result = $this->db->get('user')->row();
     if($result){
        $output = array(
          'wallet' => $result->wallet + $d->req_amount
        );
        $this->db->where('id',$user_id); 
        $this->db->update('user', $output);
      }
      $data = array(
        'notes' =>sanitizer($this->input->post('note')),
        'status' => 'rejected'

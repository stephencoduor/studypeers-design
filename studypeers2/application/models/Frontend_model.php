...
class Frontend_model extends CI_Model {
    ...
    function get_listings() {
        ...
        function filter_listing($category_ids = array(), $amenity_ids = array(), $city_id = "", $locality_id = "", $price_range = 0) {
        $listing_ids = array();
        $listing_ids_with_video = array();
        $listing_ids_with_this_city = array();
        $listing_ids_with_this_locality = array();
        ...
        foreach ($listings as $listing) {
            ...
            if(!array_diff($category_ids, $categories) && !array_diff($amenity_ids, $amenities)) {
                ...
                //Push the listing id if the listing has this locality id
                if ($locality_id != "" && $locality_id != "all") {
                  if ($listing['locality_id'] == $locality_id) {
                    if (!in_array($listing['id'], $listing_ids_with_this_locality)) {
                      array_push($listing_ids_with_this_locality, $listing['id']);
                    }
                  }
                }else {
                  array_push($listing_ids_with_this_locality, $listing['id']);
                ...
                if ($city_id == "" && $price_range == 0 && $locality_id == "") {
                  if (!in_array($listing['id'], $listing_ids)) {
                    array_push($listing_ids, $listing['id']);
                  }
                }else {
                    $listing_ids = array_intersect($listing_ids_with_video, $listing_ids_with_this_city, $listing_ids_with_this_locality, $listing_ids_with_this_price_range);
                    //print_r($listing_ids_with_video);
                    // print_r($listing_ids_with_this_locality);
    ...
    function filter_rental_listing($category_ids = array(), $amenity_ids = array(), $city_id = "", $locality_id = "", $price_range = 0) {
        $listing_ids = array();
        $listing_ids_with_video = array();
        $listing_ids_with_this_city = array();
        $listing_ids_with_this_locality = array();
        ...
        $this->db->where('status', 'active');
        $listings = $this->db->get('rental_listing')->result_array();
        foreach ($listings as $listing) {
            $categories = json_decode($listing['categories']);
            $amenities  = json_decode($listing['amenities']);
    ...
    function job_filter_listing($category_ids = array(), $city_id = "", $locality_id = "", $price_range = 0) {
        $listing_ids = array();
        $listing_ids_with_this_city = array();
        $listing_ids_with_this_price_range = array();
        $listing_ids_with_this_locality = array();
        ...
        $this->db->where('status', 'active');
        $listings = $this->db->get('job_listing')->result_array();

        foreach ($listings as $listing) {
            $categories = json_decode($listing['categories']);

            if(!array_diff($category_ids, $categories)) {
                //array_push($listing_ids_with_video, $listing['id']);
                ...
                // Push the listing id if it lies in this price range
                if ($price_range > 0) {
                  $returned_listing_id = $this->check_if_job_listing_lies_in_salary_price_range($listing['id'], $price_range);
                  if ($returned_listing_id > 0) {
                    if (!in_array($returned_listing_id, $listing_ids_with_this_price_range)) {
                      array_push($listing_ids_with_this_price_range, $returned_listing_id);
                    }
                ...
                // If with video checkbox and city checkbox remain unchecked
                if ($city_id == "" && $price_range == 0) {
                  if (!in_array($listing['id'], $listing_ids)) {
                    array_push($listing_ids, $listing['id']);
                  }
                }else {
                    $listing_ids = array_intersect($listing_ids_with_this_city, $listing_ids_with_this_locality, $listing_ids_with_this_price_range);
    ...
    function post_review() {
        ...
        $data['reviewer'] = sanitizer($this->input->post('reviewer'));
        $data['reviewer_email'] = sanitizer($this->input->post('reviewer_email'));
    ...
    function search_listing($search_string = '', $selected_category_id = '', $selected_city_id ='') {
        if ($search_string != "") {
    ...
    function get_job_category_wise_listings($category_id = "") {
        $listing_ids = array();
        $listings = $this->get_job_listings()->result_array();
        foreach ($listings as $listing) {
            // if(!has_package($listing['user_id']) > 0){
            //     continue;
            // }
    ...
    function get_top_ten_job_listings() {
        $this->db->where('status', 'active');
        $this->db->limit(10);
        return $this->db->get('job_listing');
    }

    function get_the_maximum_salary_limit_of_all_listings() {
      $related_tables = array('hotel_room_specification', 'food_menu', 'product_details');
      $maximum_prices = array();
      
        $prices = array();
        $this->db->select('salary_from');
        $query_price = $this->db->get('job_listing')->result_array();
        foreach ($query_price as $query) {
            array_push($prices, $query['salary_from']);
        }
        if (count($prices) > 0) {
          array_push($maximum_prices, max($prices));
      }else {
    ...
    function check_if_job_listing_lies_in_salary_price_range($listing_id = "", $price_range = "") {

      $maximum_price = 0;

      if ($price_range > 0) {
        $listing_details = $this->db->get_where('job_listing', array('id' => $listing_id))->row_array();

        $this->db->select_max('salary_from');
        $maximum_price = $this->db->get_where('job_listing', array('id' => $listing_id))->row()->salary_from;
    ...
    function get_top_ten_rental_listings() {
        $this->db->where('status', 'active');
        $this->db->limit(10);
        return $this->db->get('rental_listing');
    }

    function job_search_listing($search_string = '', $selected_category_id = '', $selected_city_id ='') {
        if ($search_string != "") {
            $this->db->like('name', $search_string);
        }
    ...
    public function toggle_job_wishlist($listing_id = "") {
        $existing_wishlist = array();
        $status = "";
        $user_details = $this->db->get_where('user', array('id' => $this->session->userdata('user_id')))->row_array();
        if ($user_details['job_wishlist'] != "") {
            $existing_wishlist = json_decode($user_details['job_wishlist']);
            if (in_array($listing_id, $existing_wishlist)) {
                if (($key = array_search($listing_id, $existing_wishlist)) !== false) {
                    unset($existing_wishlist[$key]);
                }
        ...
        $updater = array(
            'job_wishlist' => json_encode($existing_wishlist)
        );
        $this->db->where('id', $this->session->userdata('user_id'));
        $this->db->update('user', $updater);
        return $status;
    ...
    function get_rental_listing_wise_rating($listing_id = "") {
        $this->db->select_avg('review_rating');
        $rating = $this->db->get_where('rental_review', array('listing_id' => $listing_id))->row()->review_rating;
        return number_format((float)$rating, 1, '.', '');
    }

    function get_rental_rating_wise_quality($listing_id = "") {
        $rating = $this->get_rental_listing_wise_rating($listing_id);
        $this->db->where('rating_to >=', $rating);
        $this->db->where('rating_from <=', $rating);
        return $this->db->get('review_wise_quality')->row_array();
    }

    function get_rental_listing_wise_review($listing_id = "") {
        return $this->db->get_where('rental_review', array('listing_id' => $listing_id))->result_array();
    }

    function rental_search_listing($search_string = '', $selected_category_id = '', $selected_city_id ='') {
        if ($search_string != "") {
            $this->db->like('name', $search_string);
        }
    ...
    function get_the_maximum_rental_price_limit_of_all_listings() {
      $maximum_prices = array();
        $prices = array();
        $this->db->select('price');
        $query_price = $this->db->get('rental_listing')->result_array();
        foreach ($query_price as $query) {
            array_push($prices, $query['price']);
        }
        if (count($prices) > 0) {
    ...
    function check_if_this_rental_listing_lies_in_price_range($listing_id = "", $price_range = "") {

      $maximum_price = 0;

      if ($price_range > 0) {
        $listing_details = $this->db->get_where('rental_listing', array('id' => $listing_id))->row_array();

        $this->db->select_max('price');
        $maximum_price = $this->db->get_where('rental_listing', array('id' => $listing_id))->row()->price;
